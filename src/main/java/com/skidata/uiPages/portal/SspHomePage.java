...
public class SspHomePage extends TestBase {
	...
	@FindBy(how=How.XPATH,using ="//div[@class='login ng-scope']//a[@id='goToLogin']/div")
	...
	@FindBy(how=How.XPATH,using ="//a[text()='Reservation']")
	WebElement label_Reservation;

	@FindBy(how=How.XPATH,using ="//a[text()='Subscription']")
	...
	@FindBy(how=How.XPATH,using ="//div[@id='searchLocations']/div/form/input")
	...
	@FindBy(how=How.XPATH,using ="//div[@class='from-block']/div[@class='from']")
	WebElement label_From;

	@FindBy(how=How.XPATH,using ="//div[@class='to-block']/div[@class='to']")
	WebElement label_To;
	...
	@FindBy(how=How.XPATH,using ="//div[@id='dtFromDesk']//div[@class='day today']")
		...
		if(label_HomeIcon.getText().equals(_HomeIconLabel)) {
			...
			if(label_CartIcon.getText().equals(_CartIconLabel)) {
				...
				if(label_LoginIcon.getText().equals(_LoginIconLabel)) {
					log(label_LoginIcon.getText()+" is displayed in Home page");
	...
	public void clickOnSubscription() {
		...
		wh.waitForElementToBeVisible(label_Subscription, driver, 20);
		//clickOnCookiesGotItbutton();
	...
	public String searchParkingLocationsAndSelect(String _EnterLocationNameToSearch,String _SelectLocationAddress) throws InterruptedException {
		...
		if(field_SearchLocationBar.isDisplayed()) {
			...
			WebElement dropDownMenu_listOfAddress = driver.findElement(By.xpath("//ul[contains(@id,'typeahead') and @class='dropdown-menu ng-isolate-scope' and @template-url='ssp-auto-complete-template.html']"));

			wh.waitForElementToBeVisible(dropDownMenu_listOfAddress, driver, 20);

			List<WebElement> searchResults = driver.findElements(By.xpath("//ul[contains(@id,'typeahead')]//li//div[@class='ng-binding']"));
	...
	public void select_ProductSearchStartDate_InFromCalendar_Reservation() {
		...
		WaitHelper wh = new WaitHelper(driver, test);
		wh.waitForElementToBeClickable(datePicker_CurrentDateInFromCalendar, driver, 20);
...
//		}
	 ...
	 *****************************************************************************************************************
	 */
	public void select_ProductSearchEndDate_InToCalendar_Reservation(String _EndDate) {
		DateAndUniqueNoGenerateHelper dateAndUniqueNoGenHelper = new DateAndUniqueNoGenerateHelper();
		String currentHour = dateAndUniqueNoGenHelper.get_CurrentSystem_Day_Month_Year_Time_Minute_MilliSecond_AMPM("hour");
		String currentAMPM = dateAndUniqueNoGenHelper.get_CurrentSystem_Day_Month_Year_Time_Minute_MilliSecond_AMPM("AMPM");
		...
		if(currentHour.equals("11")&&currentAMPM.equals("PM")) {
			...
			driver.findElement(By.xpath("//div[@id='dtToDesk']//div[@data-day='"+_EndDate+"']")).click();
		}
		else {
			driver.findElement(By.xpath("//div[@id='dtToDesk']//div[@data-day='"+_EndDate+"']")).click();
		}
	...
	public void clickonLogin() throws InterruptedException {
		...
		List<WebElement> login = driver.findElements(By.xpath("//h1[@data-i18n='_LogIn_']"));
		if(login.size()>0) {
		
			log("User navigated to Login page");
		}
		else {
			label_LoginIcon.click();
		}
